...
function getUpper(anyArr) {
    const newArr = anyArr.map((elem) =>elem[0].toLocaleUpperCase() +elem.slice(1, -1) +elem[elem.length - 1].toLocaleUpperCase());
    return newArr;
  }
...
console.log(text.replaceAll('e','a').toLocaleLowerCase())
...
let indexOfFirst=text.toLocaleLowerCase().indexOf('r',6)
